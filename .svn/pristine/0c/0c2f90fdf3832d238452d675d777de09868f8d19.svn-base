...
{
    public class DamageLostService : IDamageLostService
    {
        public DataTable GetDamageLost(string FromDate, string Todate, string ShopId, string ProductId, string ItemId, string SupId, string StockType)
        {
            using (SqlConnection objConnection = Connection.GetConnection())
            {
                DataTable dt = new();
                string query = string.Format(@"SP_ReportShopDML");
